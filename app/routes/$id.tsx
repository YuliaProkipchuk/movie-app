...
export function meta({ loaderData }: Route.MetaArgs) {
  return [
    ...
    { name: "description", content: loaderData.description?.substring(0, 100) },
  ];
}
...
export default function MoviePage({ loaderData: movie }: Route.ComponentProps) {
  ...
  const { rating, rateMovie } = useRating(movie.rating);
  ...
  async function handleDelete() {
    try {
      ...
      setDeletingError("Failed to delete movie");
    } finally {
      setIsDeleting(false);
    }
  }
                ...
                <div className="rounded bg-gray-800 p-2">
                  <ul className="list-none space-y-4">
                    ...
                    <li className="flex">
                      ...
                      <span className="w-[calc(100%-140px)]">
                        {displayActors}
                      </span>
                    </li>
                    <li className="flex">
